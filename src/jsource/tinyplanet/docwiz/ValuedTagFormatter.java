...
// 7/17/2001 lmm	- use the super class in the constructor.

package tinyplanet.docwiz;
import java.util.StringTokenizer;

/** A TagFormatter for tagged paragraphs which must have a value associated with them
 * to print: see, author, since, and the like.
 *
 * The deprecated tag would not use this TagFormatter, since it should print even if it has
 * no description string associated with it. It uses ExistTagFormatter instead.
 *
 */
public class ValuedTagFormatter extends TagFormatter {

  public ValuedTagFormatter(TagSet t, String tagName) {
  	super(t, tagName);
//    setTagSet(t);
//    setTagName(tagName);
  }
  ...
  public String getJavaDocString() {
    ...
    for (i=0; i<totalElements; i++) {
	  ...
	  // TODO: Fix this so that it does not generate a tag for a param or throws if there
	  //		is only one word in the tag string. (It would be the param or exception name.)

      if (currentValue.trim().length()>0) {
        // there's some real text associated with the tag
